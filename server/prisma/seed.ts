import { PrismaClient, Role, GoalStatus, UoMType, Quarter, CheckInStatus } from '@prisma/client';
...
async function main() {
  ...
  const managers = await Promise.all([
    prisma.user.create({
      data: { name: 'Manager One', email: 'manager1@goalforge.com', password: managerPassword, role: Role.MANAGER, department: 'Innovation', managerId: admin.id }
    }),
    prisma.user.create({
      data: { name: 'Manager Two', email: 'manager2@goalforge.com', password: managerPassword, role: Role.MANAGER, department: 'Revenue Growth', managerId: admin.id }
    }),
    prisma.user.create({
      data: { name: 'Manager Three', email: 'manager3@goalforge.com', password: managerPassword, role: Role.MANAGER, department: 'Operational Excellence', managerId: admin.id }
    })
  ]);

  const employees = await Promise.all([
    prisma.user.create({ data: { name: 'Alice Kim', email: 'alice@goalforge.com', password: employeePassword, role: Role.EMPLOYEE, department: 'Innovation', managerId: managers[0].id } }),
    prisma.user.create({ data: { name: 'Ben Carter', email: 'ben@goalforge.com', password: employeePassword, role: Role.EMPLOYEE, department: 'Innovation', managerId: managers[0].id } }),
    prisma.user.create({ data: { name: 'Carmen Diaz', email: 'carmen@goalforge.com', password: employeePassword, role: Role.EMPLOYEE, department: 'Innovation', managerId: managers[0].id } }),
    prisma.user.create({ data: { name: 'Daniel Lee', email: 'daniel@goalforge.com', password: employeePassword, role: Role.EMPLOYEE, department: 'Revenue Growth', managerId: managers[1].id } }),
    prisma.user.create({ data: { name: 'Erika Patel', email: 'erika@goalforge.com', password: employeePassword, role: Role.EMPLOYEE, department: 'Revenue Growth', managerId: managers[1].id } }),
    prisma.user.create({ data: { name: 'Felix Wang', email: 'felix@goalforge.com', password: employeePassword, role: Role.EMPLOYEE, department: 'Revenue Growth', managerId: managers[1].id } }),
    prisma.user.create({ data: { name: 'Gia Shah', email: 'gia@goalforge.com', password: employeePassword, role: Role.EMPLOYEE, department: 'Operational Excellence', managerId: managers[2].id } }),
    prisma.user.create({ data: { name: 'Hugo Martinez', email: 'hugo@goalforge.com', password: employeePassword, role: Role.EMPLOYEE, department: 'Operational Excellence', managerId: managers[2].id } }),
    prisma.user.create({ data: { name: 'Isha Rao', email: 'isha@goalforge.com', password: employeePassword, role: Role.EMPLOYEE, department: 'Operational Excellence', managerId: managers[2].id } })
  ]);
  ...
  const sharedChild = await prisma.goal.create({
    data: {
      ...
      qualityScore: 90.0,
      qualityFeedback: sharedPrimary.qualityFeedback as any,
      lockedAt: new Date('2025-08-10T10:00:00Z')
    }
  });
