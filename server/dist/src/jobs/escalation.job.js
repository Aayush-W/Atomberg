...
async function runEscalationCheck() {
    ...
    for (const rule of rules) {
        ...
        if (rule.triggerType === 'GOAL_NOT_SUBMITTED') {
            ...
            if (new Date() < cycleStart)
                continue;
            const allEmployees = await prisma_1.prisma.user.findMany({ where: { role: 'EMPLOYEE' } });
            for (const emp of allEmployees) {
                const submitted = await prisma_1.prisma.goal.count({ where: { userId: emp.id, cycleId: cycle.id, status: { not: 'DRAFT' } } });
                if (submitted === 0) {
                    ...
                    if (daysSinceOpen >= rule.daysThreshold) {
                        ...
                        if (emp.managerId) {
                            const mgr = await prisma_1.prisma.user.findUnique({ where: { id: emp.managerId } });
                            if (mgr) {
                                await prisma_1.prisma.notification.create({ data: { userId: mgr.id, type: 'ESCALATION_GOAL_NOT_SUBMITTED', title: `${emp.name} Has Not Submitted Goals`, message: `${emp.name} has not submitted goals after ${daysSinceOpen} days.` } });
        ...
        if (rule.triggerType === 'APPROVAL_PENDING') {
            ...
            for (const g of pendingGoals) {
                if (g.user.managerId) {
                    const mgr = await prisma_1.prisma.user.findUnique({ where: { id: g.user.managerId } });
                    if (mgr) {
                        await prisma_1.prisma.notification.create({ data: { userId: mgr.id, type: 'ESCALATION_APPROVAL_PENDING', title: 'Goals Awaiting Approval', message: `${g.user.name}'s goal "${g.title}" has been pending your approval for ${rule.daysThreshold}+ days.` } });
        ...
        if (rule.triggerType === 'CHECKIN_MISSING') {
            ...
            for (const { q, open } of quarters) {
                ...
                for (const emp of employees) {
                    const approvedGoals = await prisma_1.prisma.goal.findMany({ where: { userId: emp.id, cycleId: cycle.id, status: { in: ['APPROVED', 'LOCKED'] } } });
                    if (approvedGoals.length === 0)
                        continue;
                    const checkedIn = await prisma_1.prisma.checkIn.count({ where: { userId: emp.id, quarter: q } });
                    if (checkedIn === 0) {
                        await prisma_1.prisma.notification.create({ data: { userId: emp.id, type: 'ESCALATION_CHECKIN_MISSING', title: `${q} Check-in Missing`, message: `Your ${q} check-in is overdue for ${cycle.name}.` } });
