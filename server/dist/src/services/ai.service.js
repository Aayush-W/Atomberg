...
async function completeJson(system, userContent) {
    if (!client) {
        return null;
    }
...
async function completeText(system, userContent, maxTokens = 900) {
    if (!client) {
        return null;
    }
    const message = await client.messages.create({
...
function inferAutopilotKey(jobTitle, department) {
    const haystack = `${jobTitle} ${department ?? ''}`.toLowerCase();
    if (haystack.includes('manager') || haystack.includes('lead'))
        return 'manager';
    if (haystack.includes('revenue') || haystack.includes('sales') || haystack.includes('account'))
        return 'revenue';
    return 'engineer';
}
function fallbackConflictCheck(goals) {
    const conflicts = [];
...
async function smartRewrite(thrustArea, title, description) {
    const ai = await completeJson('You are a goal-writing coach. Rewrite rough goals into SMART goals. Return JSON only: { smartVersion: string, scores: { specific: number, measurable: number, achievable: number, relevant: number, timeBound: number }, suggestions: string[] }', `Thrust Area: ${thrustArea}\nTitle: ${title}\nDescription: ${description}`);
    if (ai) {
        return ai;
    }
...
async function conflictCheck(goals) {
    const ai = await completeJson('You are an OKR expert. Review the provided goals and identify conflicts or direct tradeoffs. Return JSON only: { conflicts: [{ goal1Index: number, goal2Index: number, reason: string, severity: "HIGH"|"MEDIUM"|"LOW" }] }', JSON.stringify(goals));
    return ai ?? fallbackConflictCheck(goals);
}
async function suggestWeightage(goals, thrustAreas) {
    const ai = await completeJson('You are an HR expert. Suggest a weightage distribution that adds to exactly 100%. Return JSON only: { suggestions: [{ goalIndex: number, suggestedWeightage: number, rationale: string }] }', JSON.stringify({ goals, thrustAreas }));
    if (ai) {
        return ai;
    }
...
async function conversationalCheckin(goalId, quarter, messages) {
    ...
    const message = await client.messages.create({
        model: 'claude-sonnet-4-20250514',
        max_tokens: 500,
        system: 'You are a friendly HR assistant conducting a quarterly goal check-in. Ask for measurable progress, current status, blockers, and next steps. When enough information is available, include JSON with actualValue, status, blockers, and employeeNote.',
...
async function goalSummary(employeeGoalsData) {
    if (client) {
        const message = await client.messages.create({
            model: 'claude-sonnet-4-20250514',
            max_tokens: 500,
            system: 'You are an HR analyst. Write a concise three-sentence summary of goal health, strengths, risks, and recommended manager actions.',
    ...
    return {
        summary: 'The employee has a mixed portfolio with visible momentum on core delivery goals. The primary risks are approval blockers, conflicting priorities, or low check-in consistency on sensitive goals. The manager should reinforce measurable milestones and remove the highest-impact execution blockers first.'
    };
}
async function goalAutopilot(jobTitle, department) {
    const ai = await completeJson('You are a performance management expert. Based on a job title and optional department, generate exactly 5 SMART goals. Weightages must sum to exactly 100. Return JSON only: { goals: [{ thrustArea, title, description, uomType, target, targetDate, weightage, rationale }] }', JSON.stringify({ jobTitle, department }));
    if (ai?.goals?.length === 5) {
        return ai;
    }
    return { goals: AUTOPILOT_LIBRARY[inferAutopilotKey(jobTitle, department)] };
}
async function draftPerformanceReview(payload) {
    const prompt = JSON.stringify(payload);
    const text = await completeText('You are an empathetic but objective performance review assistant. Write exactly three short paragraphs: 1) overall impact and strengths, 2) measurable evidence with balanced context, 3) growth areas and next-step coaching. Avoid cliches and do not mention AI.', prompt, 1000);
    if (text) {
        return {
            draft: text,
            highlights: [
...
function overlapScore(command, title) {
...
async function parseChatOpsCommand(command, goals) {
    const ai = await completeJson('You are a workflow bot that converts Teams or Slack commands into goal updates. Match only against the provided goals and return JSON only: { intent, goalId, value, status }.', JSON.stringify({ command, goals }));
    if (ai) {
        return ai;
    }
