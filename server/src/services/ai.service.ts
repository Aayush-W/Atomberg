...
export async function conflictCheck(goals: any[]) {
  const msg = await client.messages.create({
    model: 'claude-sonnet-4-20250514',
    max_tokens: 1000,
...
export async function suggestWeightage(goals: any[], thrustAreas: string[]) {
  const msg = await client.messages.create({
    model: 'claude-sonnet-4-20250514',
    max_tokens: 1000,
...
export async function conversationalCheckin(goalId: string, quarter: string, messages: any[]) {
  const msg = await client.messages.create({
    model: 'claude-sonnet-4-20250514',
    max_tokens: 500,
...
export async function goalSummary(employeeGoalsData: any) {
  const msg = await client.messages.create({
    model: 'claude-sonnet-4-20250514',
    max_tokens: 500,
  ...
  return { summary: (msg.content[0] as any).text };
}
